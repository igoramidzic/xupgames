...
- If a design cannot reasonably be expressed with Tailwind—for example, complex keyframes, pseudo-elements, canvas behavior, or third-party internals—create a CSS file beside the owning component, import it from that component, and scope every selector under that component's root class. A component CSS file must not style another component.
...
- Keep shared room chrome and multiplayer UI game-neutral. Put drawing- and trivia-specific components in their respective game areas, with `gameType` remaining the boundary between shared room infrastructure and game implementations.
