...
vi.mock('@/games/official/prompt-arcade/PromptArcadeRoom', () => ({ default: () => <div>Prompt Arcade room</div> }));
...
describe('GameRoom routing', () => {
  ...
  it('routes Prompt Arcade through its official room module', () => {
    render(
      <GameRoom
        guest={{ sessionToken: 'a'.repeat(32), displayName: 'Igor' }}
        session={{ kind: 'session', gameType: 'promptArcade' } as never}
      />
    );
    expect(screen.getByText('Prompt Arcade room')).toBeInTheDocument();
  });
